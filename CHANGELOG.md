## [3.18.0] - 2026-09-16
### Phóng to toàn màn hình bằng phím tắt (Web & App Win)
- **Zoom cả giao diện:** Giữ phím `Ctrl` rồi lăn chuột (hoặc `Ctrl + dấu cộng/trừ`, `Ctrl + số 0` để về cỡ chuẩn) để phóng to hoặc thu nhỏ toàn bộ màn hình, giống như trên trình duyệt. Mức zoom được nhớ cho lần mở sau.

## [3.17.0] - 2026-09-16
### Sửa lỗi đăng nhập, thêm thanh zoom đề thi, tách bản cài x64/x32 (Web & App Win)
- **Hết kẹt màn hình đăng nhập:** Trước đây đăng nhập sai hoặc mất mạng thì nút quay mãi, phải tắt app mở lại. Nay sau tối đa 15 giây sẽ báo lỗi rõ ràng và cho đăng nhập lại ngay.
...
- **Bản cài riêng x64/x32:** Máy Windows 32-bit nay có file cài đặt riêng, không còn bị ghi đè; cập nhật tự động vẫn hoạt động bình thường.
- **Bảo mật:** Vá các lỗ hổng thư viện ở cả bản Web và bản Windows.
...
### QA Fixes
- **TypeScript Strict Compliance:** Khắc phục triệt để các biến rác (unused variables) trên Root Portal để vượt qua tiến trình `next build` khắt khe.
- **Rules of Hooks:** Chuẩn hóa luồng thực thi React Hooks trong `AdminStatsBar` và `QuizScreen` trên cả bản Web và Windows để đảm bảo ổn định state.
