...
function useCountdown(targetDateStr?: string) {
    ...
    return { ...timeLeft, isOver, isActive };
}
...
interface PortalMaintenanceScreenProps {
    config: {
        maintenanceMessage?: string;
        maintenanceEstimatedTime?: string;
        maintenanceEndTime?: string;
        maintenanceSafetyInfo?: string;
        maintenanceContact?: string;
    };
}

export default function PortalMaintenanceScreen({ config }: PortalMaintenanceScreenProps) {
    const { maintenanceMessage, maintenanceEstimatedTime, maintenanceEndTime, maintenanceSafetyInfo, maintenanceContact } = config;
    ...
    return (
        <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans w-full z-50 fixed inset-0">
            ...
            >
                ...
                >
                    {countdown.isOver
                        ...
                        : (maintenanceMessage || 'Hệ thống đang được bảo trì để nâng cấp và mang lại trải nghiệm tốt hơn. Vui lòng quay lại sau ít phút!')
                    }
                </motion.p>
                            ...
                            {maintenanceEstimatedTime && (
                                <div className="inline-flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-full shadow-inner backdrop-blur-sm">
                                    ...
                                    <span className="text-sm text-slate-200 font-medium font-sans">
                                        Thời gian dự kiến: {maintenanceEstimatedTime}
                                    </span>
                                </div>
                            )}
                        ...
                        <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
                            <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-full">
                                ...
                                <span className="text-sm text-slate-200">
                                    {maintenanceEstimatedTime ? `Thời gian dự kiến: ${maintenanceEstimatedTime}` : 'Thời gian dự kiến: Sớm nhất có thể'}
                                </span>
                            </div>
                        </div>
                    )}
                ...
                >
                    <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-full">
                        ...
                        <span className="text-sm text-slate-200">
                            {maintenanceSafetyInfo || 'Dữ liệu của bạn an toàn 100%'}
                        </span>
                    </div>
                </motion.div>
                ...
                >
                    <p className="text-sm text-slate-400 flex items-center justify-center gap-2">
                        ...
                        Cần hỗ trợ gấp? Liên hệ: <a href={maintenanceContact?.includes('@') ? `mailto:${maintenanceContact}` : '#'} className="text-blue-400 hover:text-blue-300 transition-colors">{maintenanceContact || 'contact@daotaothuyenvien.com'}</a>
                    </p>
                </motion.div>
            </motion.div>
        </div>
