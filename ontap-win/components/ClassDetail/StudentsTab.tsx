...
const StudentsTab: React.FC<StudentsTabProps> = ({ 
  ...
  const [selectedBadgeStudent, setSelectedBadgeStudent] = useState<UserProfile | null>(null);
  ...
  return (
    <div className="space-y-6">
              ...
              >
                <div className="flex items-start justify-between gap-4 mb-4">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={() => { setSelectedBadgeStudent(st); setShowBadgeManager(true); }} className="p-2 text-gray-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-500/10 rounded-lg transition-colors" title="Huy hiệu">🏅</button>
                    {canDisableAccounts && (
                      st.status === 'disabled' ? (
                ...
                <tr key={st.id} className={`hover:bg-gray-50 dark:hover:bg-slate-800/30 transition-colors group ${st.status === 'disabled' ? 'opacity-60 bg-gray-50/30 dark:bg-slate-900/10' : ''}`}>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => { setSelectedBadgeStudent(st); setShowBadgeManager(true); }} className="p-2 text-gray-400 hover:text-amber-600 rounded-lg transition-colors" title="Huy hiệu"><span style={{fontSize: 14}}>🏅</span></button>
                      {canDisableAccounts && (
                        st.status === 'disabled' ? (
      ...
      <BadgeAdminModal
          ...
          onClose={() => { setShowBadgeManager(false); setSelectedBadgeStudent(null); }}
          userId={selectedBadgeStudent?.id || ''}
          userName={selectedBadgeStudent?.fullName || selectedBadgeStudent?.full_name || 'Học viên'}
      />
    </div>
  );
};
