...
const NotificationBanner: React.FC<NotificationBannerProps> = ({ notification, userId, onDismiss, onOpenInbox }) => {
    ...
    return (
        <div className="fixed top-20 right-4 z-40 max-w-sm w-full animate-slide-in-right cursor-pointer" onClick={onOpenInbox}>
            <div className={`rounded-xl shadow-2xl border-l-4 p-4 flex items-start gap-3 relative overflow-hidden backdrop-blur-md bg-card/90 dark:bg-slate-800/90
                ${notification.type === 'system' ? 'border-red-500' : 'border-blue-500'}
            `}>
                <button
                    onClick={(e) => { e.stopPropagation(); setVisible(false); onDismiss(); }}
                    className="absolute top-2 right-2 text-gray-400 hover:text-muted-foreground p-1"
                >
                    <FaTimes size={12} />
                </button>
                ...
                <div className="flex-1 pr-6">
                    ...
                    <p className="text-xs text-muted-foreground dark:text-gray-300 line-clamp-2">{notification.message}</p>
