...
const OverviewTab: React.FC<OverviewTabProps> = ({ course, studentCount = 0, subjectStats = [], creatorProfiles = {} }) => {
  ...
  const sortedStats = React.useMemo(() => {
    ...
    let sortableItems = [...subjectStats];
    if (sortConfig !== null) {
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead className="text-[11px] text-gray-500 uppercase bg-gray-50 dark:bg-slate-800/50 dark:text-gray-400">
                    <tr>
                      <th className="px-4 py-3 rounded-l-lg font-bold cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-700/50 transition-colors select-none group" onClick={() => handleSort('name')}>
                        <div className="flex items-center gap-1">Tên bài thi / Môn <span className="opacity-50 group-hover:opacity-100">{sortConfig?.key === 'name' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕'}</span></div>
                      </th>
                      <th className="px-2 py-3 text-center font-bold cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-700/50 transition-colors select-none group" onClick={() => handleSort('studentsCount')}>
                        <div className="flex items-center justify-center gap-1">HV Thi <span className="opacity-50 group-hover:opacity-100">{sortConfig?.key === 'studentsCount' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕'}</span></div>
                      </th>
                      <th className="px-2 py-3 text-center font-bold cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-700/50 transition-colors select-none group" onClick={() => handleSort('passedCount')}>
                        <div className="flex items-center justify-center gap-1">Đạt <span className="opacity-50 group-hover:opacity-100">{sortConfig?.key === 'passedCount' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕'}</span></div>
                      </th>
                      <th className="px-2 py-3 text-center font-bold cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-700/50 transition-colors select-none group" onClick={() => handleSort('highest')}>
                        <div className="flex items-center justify-center gap-1">Cao nhất <span className="opacity-50 group-hover:opacity-100">{sortConfig?.key === 'highest' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕'}</span></div>
                      </th>
                      <th className="px-2 py-3 text-center font-bold cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-700/50 transition-colors select-none group" onClick={() => handleSort('lowest')}>
                        <div className="flex items-center justify-center gap-1">Thấp nhất <span className="opacity-50 group-hover:opacity-100">{sortConfig?.key === 'lowest' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕'}</span></div>
                      </th>
                      <th className="px-2 py-3 text-center rounded-r-lg font-bold cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-700/50 transition-colors select-none group" onClick={() => handleSort('average')}>
                        <div className="flex items-center justify-center gap-1">Trung bình <span className="opacity-50 group-hover:opacity-100">{sortConfig?.key === 'average' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕'}</span></div>
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {sortedStats.map((stat, idx) => (
                      <tr key={idx} className="border-b dark:border-slate-800/50 last:border-0 hover:bg-gray-50 dark:hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="px-2 py-3 text-center text-indigo-500 font-bold">{stat.studentsCount !== undefined ? stat.studentsCount : '--'}</td>
                        <td className="px-2 py-3 text-center text-emerald-600 font-bold">{stat.passedCount !== undefined ? stat.passedCount : '--'}</td>
